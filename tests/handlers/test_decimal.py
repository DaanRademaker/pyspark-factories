...
def test_handle_decimal():
    ...
    value = handle_decimal(
        StructField("some_decimal", DecimalType(precision=10, scale=4), False)
    )
    ...
    for _ in range(0, 1000):
        ...
        value = handle_decimal(
            StructField(
                "some_decimal", DecimalType(precision=precision, scale=scale), False
            )
        )
...
def test_handle_decimal_0_scale():
    ...
    value = handle_decimal(
        StructField("some_decimal", DecimalType(precision=8, scale=0), False)
    )
...
def test_handle_decimal_precision_lower_than_scale():
    with pytest.raises(ValueError):
        handle_decimal(
            StructField("some_decimal", DecimalType(precision=10, scale=10), False)
        )
